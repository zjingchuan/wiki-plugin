...
import { readIndex, upsertDoc, writeIndex } from "../../lib/index-store.js";
export function registerUpdateWikilinks(server, rootDir) {
    server.registerTool("update_wikilinks", {
        ...
        if (!fs.existsSync(fullPath)) {
            return { content: [{ type: "text", text: JSON.stringify({ error: "文档不存在" }) }] };
        }
        ...
        if (newLinks.length > 0) {
            ...
            if (existingSectionRegex.test(content)) {
                content = content.replace(existingSectionRegex, (matched) => {
        ...
        if (existing) {
            const updated = { ...existing, outgoing: allLinks };
            const newIndex = upsertDoc(index, updated);
            writeIndex(rootDir, newIndex);
        }
        return {
            content: [{ type: "text", text: JSON.stringify({ added: newLinks.length, total: existingLinks.size + newLinks.length }) }],
        };
    });
}
