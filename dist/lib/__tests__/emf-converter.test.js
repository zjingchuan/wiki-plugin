...
import { detectTool, isEmfFile, resetToolCache, SETUP_HINT, convertBatch, } from "../emf-converter.js";
test("detectTool 返回合法的 ToolInfo 形状", () => {
    resetToolCache();
    const info = detectTool();
    assert.ok(info.kind === "soffice" || info.kind === "magick" || info.kind === "none", `unexpected kind: ${info.kind}`);
    if (info.kind === "none") {
...
test("convertBatch setupHint 仅在工具不可用时设置", async () => {
    ...
    try {
        ...
        const reportMissing = await convertBatch([path.join(tmpDir, "missing.x-emf")], { rootDir: tmpDir });
        if (reportMissing.toolUsed === "none") {
            ...
            assert.strictEqual(reportMissing.setupHint, undefined, "工具可用但单项失败不应有 setupHint");
        }
    }
    finally {
        fs.rmSync(tmpDir, { recursive: true, force: true });
