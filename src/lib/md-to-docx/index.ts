...
import { tokensToDocxBlocks, type ParserContext } from "./parser.js";
...
interface ProcessedDoc {
  ...
  mermaidImages: Map<number, string>;
  mdFileDir: string;
  warnings: string[];
}
...
async function processMarkdownFile(
  ...
  const mermaidImages = new Map<number, string>();
...
function extractTitle(content: string, input: string): string {
...
async function buildDocx(doc: ProcessedDoc, exportsDir: string, headingOffset: number): Promise<string> {
  const ctx: ParserContext = {
    imageBaseDir: doc.mdFileDir,
    imageResolver: createImageResolver(
      path.resolve(exportsDir, "..", ".."),
      doc.mdFileDir,
      doc.mermaidImages,
    ),
...
async function buildMergedDocx(
  docs: ProcessedDoc[],
  exportsDir: string,
  outputName?: string,
): Promise<string> {
  ...
  for (let i = 0; i < docs.length; i++) {
    ...
    const children = tokensToDocxBlocks(doc.tokens, ctx);
    allChildren.push(...(children as InstanceType<typeof Paragraph>[]));
  }
